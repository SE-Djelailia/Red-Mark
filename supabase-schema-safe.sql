-- ============================================
-- RedMark — Script de mise à jour idempotent
-- Peut être exécuté plusieurs fois sans risque.
-- Les tables et données existantes ne sont pas effacées.
-- ============================================

CREATE EXTENSION IF NOT EXISTS "uuid-ossp";

-- ============================================
-- TABLES (IF NOT EXISTS = sûr à relancer)
-- ============================================
...
-- TABLE CRITIQUE : issues (déficiences)
-- Probablement manquante si les déficiences étaient en localStorage.
...
-- ============================================
-- RLS (enable est idempotent, pas les policies)
-- ============================================
...
-- ============================================
-- POLICIES — DROP IF EXISTS avant de recréer
-- ============================================
...
CREATE POLICY "Users can view members of their projects" ON public.project_members FOR SELECT
  USING (
...
CREATE POLICY "Users can view visits of their projects" ON public.site_visits FOR SELECT
  USING (auth.uid() = user_id OR EXISTS (SELECT 1 FROM public.project_members WHERE project_members.project_id = site_visits.project_id AND project_members.user_id = auth.uid()));
CREATE POLICY "Users can create visits for their projects" ON public.site_visits FOR INSERT
  WITH CHECK (EXISTS (SELECT 1 FROM public.projects WHERE projects.id = site_visits.project_id AND (projects.user_id = auth.uid() OR EXISTS (SELECT 1 FROM public.project_members WHERE project_members.project_id = site_visits.project_id AND project_members.user_id = auth.uid()))));
...
CREATE POLICY "Users can view photos of their projects" ON public.photos FOR SELECT
  USING (auth.uid() = user_id OR EXISTS (SELECT 1 FROM public.project_members WHERE project_members.project_id = photos.project_id AND project_members.user_id = auth.uid()));
...
CREATE POLICY "Users can view issues of their projects" ON public.issues FOR SELECT
  USING (auth.uid() = user_id OR EXISTS (SELECT 1 FROM public.project_members WHERE project_members.project_id = issues.project_id AND project_members.user_id = auth.uid()));
CREATE POLICY "Users can create issues for their projects" ON public.issues FOR INSERT
  WITH CHECK (EXISTS (SELECT 1 FROM public.projects WHERE projects.id = issues.project_id AND (projects.user_id = auth.uid() OR EXISTS (SELECT 1 FROM public.project_members WHERE project_members.project_id = issues.project_id AND project_members.user_id = auth.uid()))));
...
CREATE POLICY "Users can view comments on photos they can access" ON public.comments FOR SELECT
  USING (EXISTS (SELECT 1 FROM public.photos WHERE photos.id = comments.photo_id AND (photos.user_id = auth.uid() OR EXISTS (SELECT 1 FROM public.project_members WHERE project_members.project_id = photos.project_id AND project_members.user_id = auth.uid()))));
CREATE POLICY "Users can create comments on accessible photos" ON public.comments FOR INSERT
  WITH CHECK (EXISTS (SELECT 1 FROM public.photos WHERE photos.id = comments.photo_id AND (photos.user_id = auth.uid() OR EXISTS (SELECT 1 FROM public.project_members WHERE project_members.project_id = photos.project_id AND project_members.user_id = auth.uid()))));
...
DROP TRIGGER IF EXISTS set_updated_at_site_visits ON public.site_visits;
CREATE TRIGGER set_updated_at_site_visits BEFORE UPDATE ON public.site_visits FOR EACH ROW EXECUTE FUNCTION public.handle_updated_at();

DROP TRIGGER IF EXISTS set_updated_at_issues ON public.issues;
CREATE TRIGGER set_updated_at_issues BEFORE UPDATE ON public.issues FOR EACH ROW EXECUTE FUNCTION public.handle_updated_at();

-- ============================================
-- TERMINÉ — toutes les tables, RLS et triggers
-- sont à jour sans toucher aux données existantes.
-- ============================================

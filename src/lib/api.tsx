...
import { savePhotoToDB, getPhotosFromDB, deletePhotoFromDB, updatePhotoTagsInDB } from './indexedDB';
 ...
 */
export async function getPhotos(visitId: string): Promise<Array<{
  id: string;
  file_url: string;
  tags: string[];
  location?: string;
 ...
 */
export async function savePhotos(visitId: string, photos: Array<{
  id: string;
  file_url: string;
  tags: string[];
  uploaded_at: string;
 ...
 */
export async function createComment(visitId: string, commentData: {
  text: string;
}): Promise<Comment> {

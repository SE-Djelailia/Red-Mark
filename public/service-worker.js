...
const PRECACHE_ASSETS = ["/", "/index.html", "/app/projects", "/app/dashboard", "/offline.html"];
...
self.addEventListener("fetch", (event) => {
  ...
  if (url.pathname.startsWith("/icons/icon-")) {
    event.respondWith(
      caches.match(request, { cacheName: ICONS_CACHE }).then((response) => {
        if (response) {
          return response;
        }
...
self.addEventListener("push", (event) => {
  ...
  event.waitUntil(self.registration.showNotification("RedMark", options));
});

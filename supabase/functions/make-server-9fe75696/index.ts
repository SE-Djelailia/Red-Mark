...
// ---------------------------------------------------------------------------
// LEGACY kv_store ROUTES — DELETED
//
// 25 routes here used to read and write `kv_store_9fe75696`, a parallel store
// left over from an earlier architecture. They were disabled behind a flag for
// a while; they are now gone.
//
// WHY DELETED RATHER THAN LEFT DISABLED
//
// They could never be made safe. kv entries carry no organization_id and their
// ids do not correspond to rows in `projects`, so there was nothing to check a
// firm against — no way to firm-scope them at all. Reachable, they let any
// authenticated user read and write that store by guessing ids, with no RLS
// backstop, since every route in this file runs on the service role.
//
// Two were worse still: /debug/users/:id and /debug/test-auth carried NO
// requireAuth. /debug/users/:id handed a user profile to anyone who asked.
// A disabled unauthenticated debug route is one edit away from being a live
// one, and the flag guarding it was a single boolean. That is not a state to
// leave code in.
//
// Nothing called them: the client's only callers of this function are
// voiceNotesApi.ts (5 routes), organizationApi.ts and platformApi.ts.
//
// Six more went with them, dead in the same way but never behind the flag:
//
//   GET/POST /projects/:projectId/members       kv `user_projects:` — the worst
//     of the set. The POST accepted a user id and a role, returned
//     {success:true}, and wrote to a store NOTHING reads. It looked exactly
//     like granting project access while granting none. Real project access is
//     `project_members` (composite-FK'd to the owning firm, RLS-enforced);
//     it is managed by src/lib/firmProjectsApi.ts and by
//     /projects/:projectId/invite below. The GET was worse still in its way —
//     a hardcoded [] with a comment admitting it never worked.
//
//   GET/POST /projects/:projectId/floor-plans   kv `floor_plan:` — superseded
//     by the `plan_files` / `plans` / `pin_placements` tables, which is what
//     src/lib/plansApi.ts and PlanFileViewer use.
//
//   GET /test                                   unauthenticated liveness echo,
//     duplicating /health. Two of these is one too many.
//
//   POST /auth/signup                           already a 410 tombstone. It had
//     called admin.createUser({email_confirm:true}) UNAUTHENTICATED, handing
//     any anonymous caller an account with a pre-confirmed address — which
//     would have defeated the invitation handshake, since /organizations/claim
//     grants firm membership on a verified-email match. Accounts are created by
//     invitation (/organizations/invitations) or provisioning
//     (/organizations/members/provision), both firm-admin gated.
//
// WHAT REMAINS OF THE KV STORE: voice notes, and nothing else. Every `kv.*`
// call left in this file is a `voice_note:` or `visit_voice_notes:` key.
//
// Deliberately NOT recreated in any form. If something needs project or photo
// data, it goes through Postgres with RLS, like the rest of the app.
